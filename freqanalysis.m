...
for i = 1:length(params.trigger_code)
    cfg = [];
    cfg.trials = find(data.trialinfo==params.trigger_code(i));
...
%     save(fullfile(save_path, [params.sub '_' params.modality '_tfr_dpss']), 'tfr_dpss',"-v7.3"); disp('done');
    
    %%
    % Plot TFR
%     load(fullfile(save_path, [params.sub '_' params.modality '_tfr_dpss.mat']));
    ...
    colorbar
    saveas(h, fullfile(save_path, 'figs', [params.sub '_' params.modality '_TFR multi_' params.trigger_labels{i} '.jpg']))
...
% save(fullfile(save_path, [params.sub '_' params.modality '_tfr_wavelet']), 'tfr_wavelet',"-v7.3"); disp('done');
% 
% %%
% load(fullfile(save_path, [params.sub '_' params.modality '_tfr_wavelet.mat']));
% 
% % Plot TFR
% cfg = [];
% cfg.parameter       = 'powspctrm';
...
% colorbar
% saveas(h, fullfile(save_path, 'figs', [params.sub '_' params.modality '_TFR wave_' params.trigger_labels{i} '.jpg']))
% 
% % Channel with highest peak
% cfg.channel = pks;
% ft_singleplotTFR(cfg, tfr_dpss)
